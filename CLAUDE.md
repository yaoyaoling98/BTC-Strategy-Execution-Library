...
**打分系统：** RSI 25分 + BB 25分 + VWAP 25分 + MFI 25分 = 100分（V底+10上限至100，RSI斜率不对-15）  
**信号阈值：** ≥65分才出信号；≥85分为强信号  
...
> **唯一性原则：所有升级与优化只在 `btc-signal-v2.html` 中进行，不新建任何文件。**

1. **唯一维护对象**：`btc-signal-v2.html` 是唯一代码文件，禁止创建备份文件或拆分模块
2. **大改动分块进行**：每次改动不超过200行，改完验证再继续
3. **不动算法逻辑**：UI/CSS改动不得修改任何计算函数
4. **改完必须推送**：每次改动 commit + push 到 `main` 分支（同步到 `claude/general-work-3znly`）
5. **注释算法来源**：新增指标代码必须注明来源（Jesse/Freqtrade/自研）
6. **内置回测验证流程**：修改任何策略函数后，点击页面「回测」按钮跑一遍90天，
   确认期望值为正再 commit。回测数据来自 OKX，实盘来自 Kraken，
   价差约0.1-0.3%，不影响策略有效性判断。
...
> **唯一性硬规则**：根目录不得出现其他 `.html` 文件，不得新建 `.js` / `.mjs` / `.ts` 模块。
> 所有新功能均在 `btc-signal-v2.html` 内扩展，永远单文件交付。
